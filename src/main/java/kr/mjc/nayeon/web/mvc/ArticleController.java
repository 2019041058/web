...
public class ArticleController {
    ...
    /**
     * 게시글 쓰기 화면. 로그인 안 했을 경우 로그인 화면으로 변환
     */
    ...
    /**
     * 게시글 수정 화면. 로그인 안 했으면 로그인 화면으로, 사용자가 다르면 401 Unauthorized.
     */
    public void articleUpdate(HttpServletRequest request,
                            HttpServletResponse response)
            throws IOException, ServletException {
        HttpSession session = request.getSession();
        // 로그인 체크
    ...
    /**
     * 게시글 삭제 액션. 로그인 안 했으면 로그인 화면으로 redirect.
     * 사용자가 다르면 401
     */
